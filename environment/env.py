...
class Environment:
    ...
    Table2Pos = [0.7, 0.0, 0.685]
    SIMULATION_STEP_DELAY = 0.0000000005 #speed of simulator - the lower the fatser ## should be a param
    FINGER_LENGTH = 0.06
    Z_TABLE_TOP = 0.785
    GRIP_REDUCTION = 0.60

    def __init__(self, camera: Camera, exampleCamera: Camera, vis=False, debug=False, gripper_type='140', finger_length=0.06) -> None:
        self.vis = vis
        self.debug = debug
        self.camera = camera
        ...
        self.example_table_id = p.loadURDF('environment/urdf/objects/table.urdf',
                                  [0.0, 0.65, 0.76],
                                  p.getQuaternionFromEuler([0, 0, 0]),
                                  useFixedBase=True)
        self.example_table_extension_id = p.loadURDF('environment/urdf/objects/table.urdf',
                                  [0.0, 1.0, 0.76],
                                  p.getQuaternionFromEuler([0, 0, 0]),
                                  useFixedBase=True)
        ...
        if vis:
            ...
            self.drawCamera(camera, alpha, "RGB-D camera")
            self.drawCamera(exampleCamera, alpha, "Example camera")
    ...
    def drawCamera(self, camera, alpha, caption):
        z_cam_l1 = camera.z #m 

        color = [0.3, 0, 0]
        p.addUserDebugText(caption, [camera.x-0.1, camera.y, camera.z+0.02], color, textSize=2)
    ...
    def reset_robot(self):
        ...
        #user_parameters = (-1.5707970583733368, -1.5707970583733368, 0.0, -1.5707970583733368, 0.0, 1.5707970583733368, 0.085)
        for _ in range(60):
            for i, name in enumerate(self.controlJoints):
                ...
                p.setJointMotorControl2(self.robot_id, joint.id, p.POSITION_CONTROL,
                                        targetPosition=user_parameters[i], force=100.0, #joint.maxForce,
                                        maxVelocity= 20.0)#joint.maxVelocity)
    ...
    def create_example_pile(self, obj_info, added_y):
        ...
        for path, mod_orn, mod_stiffness in obj_info:
            ...
            r_y = random.uniform(
                self.obj_init_pos[1] - margin + added_y, self.obj_init_pos[1] + margin + added_y)
            yaw = random.uniform(0, np.pi)
            pos = [r_x, r_y, 1.0]
